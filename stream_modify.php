...
*******************************************************************************
**                           stream_modify.php                               **
** Example of how to modify a stream using the ScaleEngine API.              **
** Other objects can be modified using the same method, by calling the       **
...
$timeout = 180;
$endpoint = 'streams';
$object = $stream_name; // use stream_name to select object to modify
$method = 'POST'; // Use post method to modify


$data = array(
    "user"		=> "myuser",  //modify the stream to add a username and password
    "pass"		=> "12345", // other settings will not be changed
);
$body = json_encode($data, JSON_FORCE_OBJECT | JSON_THROW_ON_ERROR);
